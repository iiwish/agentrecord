...
```
*(或者在源码库中通过 `node src/cli.mjs` 运行命令行)*
...
| 命令 | 功能说明 | 常用参数与示例 |
| :--- | :--- | :--- |
| `init` | 初始化本地配置 Schema。 | `--owner <id> --display-name "名字" --dry-run` |
| `scan` | 探测支持的 Agent 工具及其本地数据库。 | `--sessions-dir ~/.codex/sessions` |
| `build` | 编译生成 JSON、MD 及 HTML 画像。 | `--owner <id> --locale zh-CN --agent-context` |
| `validate` | 校验隐私安全性、Schema 规范及国际化对齐。 | `--config ./agentrecord.config.json` |
| `open` | 在浏览器中安全打开生成的 HTML 护照页面。 | 自动寻址打开 `profiles/<owner>/index.html` |
